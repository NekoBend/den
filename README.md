...
- `den/`        the `den` CLI package: a dependency-free `uv` tool that bundles the deployable content and installs it (`install`, `check`, `verify`, `refs`, `doc`, `memory`, `hook`, `cheat`). See `den/README.md`.
