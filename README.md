...
CLI, and `shell/README.md` for the shell environment. [`MEASUREMENTS.md`](MEASUREMENTS.md)
records what has been measured about the prompts and what was decided from it. [`COMMANDS.md`](COMMANDS.md)
...
- `agents/`      LLM skill system: `src/` (9 hand-authored skills + shared resources) and `dist/` (generated parent prompts; never hand-edited). See `agents/README.md`.
...
`.github/workflows/ci.yml` runs six jobs: `agents` (ruff, `ty` type-check, a
...
sourceless install), `shell` (shellcheck + PSScriptAnalyzer, errors only),
`shell-tests` (the bash/zsh/pwsh functional tests in Docker), and `windows`
(pytest + a pwsh init smoke on a native windows-latest runner).
