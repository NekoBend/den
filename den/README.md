...
that resolves inside the workspace - a cloned repo shipping its own `ruff.exe`,
which Windows would otherwise prefer over the installed one - is refused and
counted as a `SKIP`, never executed.
